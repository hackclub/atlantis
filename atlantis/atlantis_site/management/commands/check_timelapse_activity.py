"""Run inactivity detection over compiled timelapse footage.

Meant to be run on a timer, the way submit_airtable is. Every complete recording
...
class Command(BaseCommand):
    help = "Analyse compiled timelapse videos for stretches where nothing changes."
    ...
    def handle(self, *args, **options):
        ...
        for session in sessions:
            ...
            if result is None:
                failed += 1
                self.stderr.write(
                    f"timelapse {session.id} ({session.external_id}): "

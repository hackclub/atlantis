...
@check_perms(["atlantis_site.printer", "atlantis_site.organizer"])
...
def claim_print(request, ship_id):
    ...
    if owner_slack_id:
        send_slack_dm(f"Your project <https://atlantis.hacklub.com/projects/{ship.project.id}|{ship.project.title}> is being printed!", owner_slack_id)
...
def unclaim_print(request, ship_id):
    ...
    owner_slack_id = ship.project.owner.hackclub_profile.slack_id
    if owner_slack_id:
        send_slack_dm(f"Your project <https://atlantis.hacklub.com/projects/{ship.project.id}|{ship.project.title}> is no longer being printed!", owner_slack_id)
...
def print_decision(request, ship_id):
    ...
    owner_slack_id = ship.project.owner.hackclub_profile.slack_id
    if owner_slack_id:
        send_slack_dm(f"Your project <https://atlantis.hacklub.com/projects/{ship.project.id}|{ship.project.title}> has been printed and {"sent back to T1" if decision == Print.Decision.RETURN_T1 else "approved"}! Here's what they said about it: _{feedback}_", owner_slack_id)
